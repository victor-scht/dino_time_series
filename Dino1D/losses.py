...
def dino_loss_multicrop_2global(
    ...
    p0 = tg[0]
    loss0 = (
        -(p0 * log_sg[1]).sum(-1).mean()
        + n_local * (-(p0[None] * log_sl).sum(-1).mean())
    ) / (1 + n_local)

    p1 = tg[1]
    loss1 = (
        -(p1 * log_sg[0]).sum(-1).mean()
        + n_local * (-(p1[None] * log_sl).sum(-1).mean())
    ) / (1 + n_local)
